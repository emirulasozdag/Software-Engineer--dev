...
const Assignments: React.FC = () => {
  ...
  const open = (studentAssignmentId: number) => {
    navigate(`/student/assignments/${studentAssignmentId}`);
  };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginTop: '20px' }}>
          ...
          {pending.map((a) => (
            <div key={a.studentAssignmentId} className="card" style={{ background: '#fff3cd', borderLeft: '4px solid #ffc107' }}>
              ...
              <button className="button button-primary" style={{ marginTop: '10px' }} onClick={() => open(a.studentAssignmentId)}>
                Open Assignment
              </button>
            </div>
          ))}
          ...
          {completed.map((a) => (
            <div key={a.studentAssignmentId} className="card" style={{ background: '#d4edda', borderLeft: '4px solid #28a745' }}>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
                <button className="button button-secondary" onClick={() => open(a.studentAssignmentId)}>
                  View Details
                </button>
